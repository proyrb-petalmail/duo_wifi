...
static char lv_pinyin_k9_cand_str[LV_IME_PINYIN_K9_CAND_TEXT_NUM + 2][LV_IME_PINYIN_K9_MAX_INPUT] =
    {0};
#endif
...
void lv_ime_pinyin_set_keyboard(lv_obj_t *obj, lv_obj_t *kb)
{
    ...
    lv_obj_set_parent(obj, lv_obj_get_parent(pinyin_ime->kb));
    lv_obj_set_parent(pinyin_ime->cand_panel, lv_obj_get_parent(pinyin_ime->kb));
...
void lv_ime_pinyin_set_mode(lv_obj_t *obj, lv_ime_pinyin_mode_t mode)
{
    ...
    if (pinyin_ime->mode == LV_IME_PINYIN_MODE_K9)
    {
        ...
        lv_keyboard_set_map(pinyin_ime->kb, LV_KEYBOARD_MODE_USER_1,
                            (const char **)lv_btnm_def_pinyin_k9_map, default_kb_ctrl_k9_map);
...
static void lv_ime_pinyin_kb_event(lv_event_t *e)
{
    ...
    if (code == LV_EVENT_VALUE_CHANGED)
    {
        ...
        const char *txt =
            lv_buttonmatrix_get_button_text(kb, lv_buttonmatrix_get_selected_button(kb));
        if (txt == NULL)
            return;
        ...
        if (pinyin_ime->mode == LV_IME_PINYIN_MODE_K9)
        {
            ...
            if ((btn_id >= 16) && (tmp_button_str_len > 0) &&
        ...
        else if ((lv_strcmp(txt, "ABC") == 0) || (lv_strcmp(txt, "abc") == 0) ||
                 (lv_strcmp(txt, "1#") == 0) || (lv_strcmp(txt, LV_SYMBOL_OK) == 0))
        {
            pinyin_ime_clear_data(obj);
            return;
        }
        ...
        else if (lv_strcmp(txt, LV_SYMBOL_KEYBOARD) == 0)
        {
            ...
            pinyin_ime_clear_data(obj);
                 ...
                 ((txt[0] >= 'a' && txt[0] <= 'z') || (txt[0] >= 'A' && txt[0] <= 'Z')))
        {
            lv_strcat(pinyin_ime->input_char, txt);
            pinyin_input_proc(obj);
            pinyin_ime->ta_count++;
...
static void pinyin_input_proc(lv_obj_t *obj)
{
    ...
    pinyin_ime->cand_str =
        pinyin_search_matching(obj, pinyin_ime->input_char, &pinyin_ime->cand_num);
        ...
        (pinyin_ime->k9_legal_py_count > LV_IME_PINYIN_K9_CAND_TEXT_NUM))
    {
        ...
        if (dir == 1)
        {
            ...
            while (ll_index)
            {
                ...
                ll_index =
                    lv_ll_get_next(&pinyin_ime->k9_legal_py_ll, ll_index); /*Find the next list*/
                count++;
            }
        ...
        else
        {
            ...
            while (ll_index)
            {
                ...
                lv_strcpy(lv_pinyin_k9_cand_str[count], ll_index->py_str);
                ll_index = lv_ll_get_prev(&pinyin_ime->k9_legal_py_ll,
