...
void lv_indev_read_timer_cb(lv_timer_t *timer) { lv_indev_read(timer->user_data); }
...
lv_indev_t *lv_indev_active(void) { return indev_act; }
...
lv_dir_t lv_indev_get_gesture_dir(const lv_indev_t *indev) { return indev->pointer.gesture_dir; }
...
lv_obj_t *lv_indev_get_active_obj(void) { return indev_obj_act; }
...
void lv_indev_add_event_cb(lv_indev_t *indev, lv_event_cb_t event_cb, lv_event_code_t filter,
                           void *user_data)
{
    LV_ASSERT_NULL(indev);
...
uint32_t lv_indev_remove_event_cb_with_user_data(lv_indev_t *indev, lv_event_cb_t event_cb,
                                                 void *user_data)
{
    LV_ASSERT_NULL(indev);
...
static void indev_keypad_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    ...
    /*Save the last key. *It must be done here else `lv_indev_get_key` will return the last key in
     * events*/
    ...
    /*Pressing*/
    else if (is_enabled && data->state == LV_INDEV_STATE_PRESSED &&
    ...
    /*Release happened*/
    else if (is_enabled && data->state == LV_INDEV_STATE_RELEASED &&
             prev_state == LV_INDEV_STATE_PRESSED)
    {
            ...
            if (data->key == LV_KEY_ENTER)
            {
                ...
                if (editable_or_scrollable)
                {
                    ...
                    if (lv_group_get_obj_count(g) > 1)
                    {
                        ...
                        lv_obj_remove_state(indev_obj_act,
...
static void indev_button_proc(lv_indev_t *i, lv_indev_data_t *data)
{
    ...
    if (LV_INDEV_STATE_RELEASED != data->state)
    {
        ...
        {
            LV_LOG_INFO("button %" LV_PRIu32 " is pressed (x:%d y:%d)", data->btn_id, (int)x,
                        (int)y);
        }
        else
        {
            LV_LOG_INFO("button %" LV_PRIu32 " is released (x:%d y:%d)", data->btn_id, (int)x,
                        (int)y);
        }
    }
    ...
    if (data->state == LV_INDEV_STATE_PRESSED)
    {
        if (i->pointer.last_point.x != x || i->pointer.last_point.y != y)
        {
            indev_proc_release(i);
        }
    }
...
static void indev_proc_press(lv_indev_t *indev)
{
    ...
    /*Calculate the vector and apply a low pass filter: new value = 0.5 * old_value + 0.5 *
     * new_value*/
    ...
    indev->pointer.scroll_throw_vect.x =
        (indev->pointer.scroll_throw_vect.x + indev->pointer.vect.x) / 2;
    indev->pointer.scroll_throw_vect.y =
        (indev->pointer.scroll_throw_vect.y + indev->pointer.vect.y) / 2;

    indev->pointer.scroll_throw_vect_ori = indev->pointer.scroll_throw_vect;

    if (LV_ABS(indev->pointer.vect.x) > indev->scroll_limit ||
        LV_ABS(indev->pointer.vect.y) > indev->scroll_limit)
    {
    ...
    if (indev_obj_act)
    {
        ...
        if (indev->mode == LV_INDEV_MODE_EVENT && indev->read_timer &&
            lv_timer_get_paused(indev->read_timer))
        {
            lv_timer_resume(indev->read_timer);
        }
    ...
    if (indev_obj_act)
    {
        ...
        if (scroll_obj)
        {
            ...
            if (angle != 0 || scale_y != LV_SCALE_NONE || scale_x != LV_SCALE_NONE)
            {
                ...
                lv_point_transform(&indev->pointer.scroll_throw_vect, angle, scale_x, scale_y,
                                   &pivot, false);
                lv_point_transform(&indev->pointer.scroll_throw_vect_ori, angle, scale_x, scale_y,
                                   &pivot, false);
            }
        }
    }
...
static void indev_proc_pointer_diff(lv_indev_t *indev)
{
    ...
    if (editable)
    {
        ...
        int32_t diff =
            (int32_t)((int32_t)indev->pointer.diff * indev_sensitivity * obj_sensitivity + 32768) >>
            16;
...
static void indev_click_focus(lv_indev_t *indev)
{
    ...
    lv_group_t *g_prev =
        indev->pointer.last_pressed ? lv_obj_get_group(indev->pointer.last_pressed) : NULL;
...
/**
 * Checks if the reset_query flag has been set. If so, perform necessary global indev cleanup
 * actions
 * @param proc pointer to an input device 'proc'
...
/**
 * Checks if the stop_processing_query flag has been set. If so, do not send any events to the
 * object
 * @param indev pointer to an input device
 * @return true if indev should stop processing the event.
 */
static bool indev_stop_processing_check(lv_indev_t *indev) { return indev->stop_processing_query; }
...
static lv_result_t send_event(lv_event_code_t code, void *param)
{
    ...
    if (code == LV_EVENT_PRESSED || code == LV_EVENT_SHORT_CLICKED || code == LV_EVENT_CLICKED ||
        ...
        code == LV_EVENT_LONG_PRESSED_REPEAT || code == LV_EVENT_ROTARY)
    {
