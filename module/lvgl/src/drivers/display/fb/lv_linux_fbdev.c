...
void lv_linux_fbdev_set_file(lv_display_t *disp, const char *file)
{
    ...
    //     /* Don't return. Some framebuffer drivers like efifb or simplefb don't implement
    //      * FBIOBLANK.*/
    // }
    ...
    LV_LOG_INFO("Resolution is set to %" LV_PRId32 "x%" LV_PRId32 " at %" LV_PRId32 "dpi", hor_res,
                ver_res, lv_display_get_dpi(disp));
}
...
static void flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *color_p)
{
    ...
    /* Not all framebuffer kernel drivers support hardware rotation, so we need to handle it in
     * software here */
    ...
    uint32_t fb_pos = (area->x1 + dsc->vinfo.xoffset) * px_size +
    ...
    {
        uint32_t color_pos = area->x1 * px_size + area->y1 * disp->hor_res * px_size;
