...
static lv_point_t _evdev_process_pointer(lv_indev_t *indev, int x, int y)
{
    ...
    lv_point_t p;
    if (0 == lv_display_get_rotation(disp) % 2)
    {
        p.x = _evdev_calibrate(swapped_x, dsc->min_x, dsc->max_x, 0, width - 1);
        p.y = _evdev_calibrate(swapped_y, dsc->min_y, dsc->max_y, 0, height - 1);
    }
    else
    {
        p.x = _evdev_calibrate(swapped_x, dsc->min_x, dsc->max_x, 0, height - 1);
        p.y = _evdev_calibrate(swapped_y, dsc->min_y, dsc->max_y, 0, width - 1);
    }
    return p;
}
