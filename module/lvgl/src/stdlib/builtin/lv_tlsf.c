...
#ifdef TLSF_64BIT
#define TLSF_FLS(n)                                                                                \
    ((n) & 0xffffffff00000000ull ? 32 + TLSF_FLS32((size_t)(n) >> 32) : TLSF_FLS32(n))
#else
...
static block_header_t *block_trim_free_leading(control_t *control, block_header_t *block,
                                               size_t size)
{
...
static block_header_t *block_locate_free(control_t *control, size_t size)
{
    ...
    if (size)
    {
        ...
        /*
        ** mapping_search can futz with the size, so for excessively large sizes it can sometimes
        *wind up
        ** with indices that are off the end of the block array.
        ** So, we protect against that here, since this is the only callsite of mapping_search.
        ** Note that we don't need to check sl, since it comes from a modulo operation that
        *guarantees it's always in range.
        */
